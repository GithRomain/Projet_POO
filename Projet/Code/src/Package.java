...
class Joueurs
{
    ...
    //Methodes
    public Joueur selection_aleatoire_joueur()
    {
        int ind_alea = new Random().nextInt(joueurs.size() - 1);
        if(joueurs.get(ind_alea).getEtat().equals("répondu") && joueurs.get(ind_alea).getEtat().equals("éliminé"))
        {
            selection_aleatoire_joueur();
        }
        else
        {
            joueurs.get(ind_alea).setEtat("sélectionné");
        }
        return joueurs.get(ind_alea);
    }

    public void generer_ensemble_participants()
    {
        List<Joueur> new_joueurs = new ArrayList<>();

        while (new_joueurs.size() != 4)
        {
            int ind = new Random().nextInt(joueurs.size() - 1);
